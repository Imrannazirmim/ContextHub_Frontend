...
const ContestDetails = () => {
    ...
    useEffect(() => {
        ...
        const fetchContest = async () => {
            try {
                ...
                const message = err?.response?.data?.message || err.message || "Failed to fetch contest details";
                setError(message);
    ...
    if (!contestData) return <span className="alert-warning">Contest not found!</span>;
            ...
            <main className="grid grid-cols-1 lg:grid-cols-12 gap-8 sticky container mx-auto pt-16 pb-8">
                ...
                <section className="lg:col-span-8 space-y-12 mx-0 p-4 lg:p-0 lg:mx-4">
                    <div className="w-full aspect-video overflow-hidden rounded-2xl">
                        <img src={contestData.image} alt={contestData.name} className="w-full h-full object-cover" />
                    </div>
                    <span className="btn btn-outline btn-primary ">{contestData.contestType}</span>
                    <div className="space-y-5">
                        <h2 className="text-2xl font-semibold text-wrap">{contestData.name}</h2>
                    ...
                    <div className="flex flex-col gap-2 flex-wrap bg-green-100 py-6 px-8 rounded-2xl">
                        <h4 className="text-xl font-semibold text-green-500">Winner Announcement</h4>
                        <div className="flex items-center gap-2">
                            <img className="w-12 h-12 rounded-full object-cover" src={contestData.image} alt="" />
                            <div>
                                <strong>{user?.displayName}</strong>
                                <p className="text-sm text-green-800">Congratulations to the winner!</p>
                            </div>
                        </div>
                    </div>
                </section>
